      ...
      <section class="bg-card-bg rounded-3xl border border-border shadow-sm overflow-visible relative">
        <div class="px-6 py-4 border-b border-border flex items-center justify-between">
          <div class="flex items-center gap-3">
             <div class="w-8 h-8 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500 text-sm">💊</div>
             <h2 class="text-xs font-black uppercase tracking-widest text-text-primary">Stock Items</h2>
          </div>
          <button @click="addLine" type="button" class="px-5 py-2 rounded-xl bg-hover-bg text-text-primary text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all flex items-center gap-2">
          ...
          <table class="w-full min-w-[1200px] border-collapse">
            ...
            <tbody>
              <tr v-for="(line, idx) in form.items" :key="idx" class="group border-b border-border/50 hover:bg-hover-bg/20 transition-colors">
                <td class="py-3 px-4 text-center text-[10px] font-black text-text-muted opacity-30">{{ idx + 1 }}</td>
                <td class="py-3 px-6">
                  <AutoCompleteWithCreate
                    v-model="line.item_id"
                    :options="inventoryStore.items.map(i => ({ id: i.id, name: i.name, sub: i.brand }))"
                    placeholder="Search medicines..."
                    allow-create
                    @create="handleCreateItem(idx, $event)"
                ...
                <td class="py-3 px-4">
                  <div class="relative">
                    ...
                    />
                    <!-- Batch Popper -->
                    <div v-if="activeBatchIdx === idx" class="fixed z-[100] bg-card-bg border border-border rounded-2xl shadow-2xl p-2 min-w-[220px]" :style="{ top: getBatchPopperPos(idx).top + 'px', left: getBatchPopperPos(idx).left + 'px' }">
                       ...
                       <div v-for="b in getItemBatches(line.item_id || '')" :key="b.id" @click="onLineBatchChange(idx, b.id)" class="p-3 rounded-xl hover:bg-hover-bg cursor-pointer transition-all border border-transparent hover:border-border">
                ...
                <td class="py-3 px-4">
                  <div class="flex flex-col items-center">
                    <input v-model="line.qtyInput" @input="parseQuantity(idx)" type="text" class="w-24 bg-hover-bg/50 border border-border rounded-xl py-1.5 text-center text-xs font-black outline-none focus:ring-2 focus:ring-brand/10" placeholder="0+0"/>
                ...
                <td class="py-3 px-4">
                  <button @click="removeLine(idx)" class="w-8 h-8 rounded-xl flex items-center justify-center text-text-muted hover:bg-rose-500 hover:text-white transition-all opacity-0 group-hover:opacity-100">
...
const activeItemLineIdx = ref<number | null>(null);

const activeBatchIdx = ref<number | null>(null);
...
function handleCreateItem(idx: number, q: string) {
  activeItemLineIdx.value = idx;
  initialSearchTerm.value = q;
  showItemModal.value = true;
}

function onItemSelect(idx: number, suggestion: any) {
...
async function onItemSubmit(payload: any) {
    ...
    if (activeItemLineIdx.value !== null && form.value.items[activeItemLineIdx.value]) {
        (form.value.items[activeItemLineIdx.value] as any).item_id = id as any;
        const item = inventoryStore.items.find(i => i.id === (id as any));
        if (item) {
             (form.value.items[activeItemLineIdx.value] as any).rate = props.type === 'Sales' ? ((item as any).sales_rate || 0) : ((item as any).purchase_rate || 0);
        }
    }
...
function onLineBatchChange(idx: number, batchId: string) {
  const line = form.value.items[idx];
...
function getBatchPopperPos(idx: number) {
  // Simple heuristic or more accurate rect finder
  const inputs = document.querySelectorAll('input[placeholder="BATCH-00x"]');
  const target = inputs[idx] as HTMLElement;
  if (target) {
...
const totals = computed(() => {
  return form.value.items.reduce((acc, l) => {
...
const isFormValid = computed(() => {
  return !!form.value.party_id && form.value.items.every(i => !!i.item_id && i.quantity > 0);
});
